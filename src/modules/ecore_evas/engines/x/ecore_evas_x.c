...
#define HANDLE_TYPE() \
   { \
      edata->seleciton_data[selection].requested_type = eina_stringshare_add(name); \
   ...
   for (unsigned int i = 0; i < eina_array_count(arr) && !found_conversion; ++i)
     {
        const char *name = eina_array_data_get(arr, i);
        mime_type = _decrypt_type(name);
        ...
        for (unsigned int j = 0; j < eina_array_count(edata->seleciton_data[selection].acceptable_type) && !found_conversion; ++j)
          {
             ...
             if (!found_conversion)
               {
                  Eina_Iterator *iter = eina_content_converter_possible_converstions(acceptable_type);
   ...
   if (found_conversion)
     ...
     {
        eina_promise_resolve(edata->seleciton_data[selection].delivery, eina_value_error_init(0));
...
_ecore_evas_x_selection_fixes_notify(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (edata->skip_changed_event)
     {
        edata->skip_changed_event --;
        goto end;
     }
...
_ecore_evas_x_selection_request(Ecore_Evas *ee EINA_UNUSED, unsigned int seat EINA_UNUSED, Ecore_Evas_Selection_Buffer selection, Eina_Array *acceptable_type)
{
   ...
   if (sdata->delivery)
     {
        eina_promise_reject(sdata->delivery, 0); //FIXME
        _clear_selection_delivery(ee, selection);
     }
