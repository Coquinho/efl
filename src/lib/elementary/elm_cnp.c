...
_elm_sel_format_to_mime_type(Elm_Sel_Format format)
{
   if (format == ELM_SEL_FORMAT_TEXT)
     return eina_stringshare_add("text/plain;charset=utf-8");
   else if (format == ELM_SEL_FORMAT_MARKUP)
     return eina_stringshare_add("application/x-elementary-markup");
   else if (format == ELM_SEL_FORMAT_IMAGE)
     return eina_stringshare_add("image/*"); //We do not specify a real type anywhere
   else if (format == ELM_SEL_FORMAT_VCARD)
     return eina_stringshare_add("text/vcard");
   else if (format == ELM_SEL_FORMAT_HTML)
     return eina_stringshare_add("application/xhtml+xml");
                                     ...
                                     const void *buf, size_t buflen)
{
   ...
   mime_type = _elm_sel_format_to_mime_type(format);
...
static Eina_List *_legacy_lost_cb_list = NULL;

void
selection_is_changed_emits(void)
{
   Sel_Lost_Data *data;

   EINA_LIST_FREE(_legacy_lost_cb_list, data)
     {
        data->loss_cb(data->udata, data->type);
        free(data);
     }
}
...
elm_cnp_selection_loss_callback_set(Evas_Object *obj, Elm_Sel_Type type, Elm_Selection_Loss_Cb func, const void *data)
{
   ...
   ldata->loss_cb = func;
   _legacy_lost_cb_list = eina_list_append(_legacy_lost_cb_list, ldata);
}
                                     ...
                                     Elm_Drop_Cb data_cb, void *udata)
{
   ...
   ee = ecore_evas_ecore_evas_get(evas_object_evas_get(obj));
   mime_type = _elm_sel_format_to_mime_type(format);
   const char *tmp[] = {mime_type};
   future = ecore_evas_selection_get(ee, 0, _elm_sel_type_to_ee_type(selection), EINA_C_ARRAY_ITERATOR_NEW(tmp));
