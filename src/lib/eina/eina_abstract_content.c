...
static Eina_Hash *convertion_callbacks;

typedef struct {
   const char *to;
   Eina_Content_Convertion_Callback callback;
} Eina_Content_Convertion_Node;
...
EAPI Eina_Bool
eina_content_converter_conversion_register(const char *from, const char *to, Eina_Content_Convertion_Callback convertion)
{
   Eina_Content_Convertion_Node *node = calloc(1, sizeof(Eina_Content_Convertion_Node));
   ...
   node->callback = convertion;

   eina_hash_list_append(convertion_callbacks, shared_from, node);

   return EINA_TRUE;
}

static inline Eina_List*
_convertion_callback_fetch_possible(const char *from)
{
   Eina_Stringshare *shared_from = eina_stringshare_add(from);
   Eina_List *res = eina_hash_find(convertion_callbacks, shared_from);
   eina_stringshare_del(shared_from);
   return res;
}

static inline Eina_Content_Convertion_Callback
_convertion_callback_fetch(const char *from, const char *to)
{
   Eina_List *possibilities = _convertion_callback_fetch_possible(from);
   Eina_Content_Convertion_Node *n;
   Eina_Content_Convertion_Callback result = NULL;
   Eina_List *l;
...
{
   return !!_convertion_callback_fetch(from, to);
}

static const void*
_process_cb(const void *container EINA_UNUSED, void *data, void *fdata EINA_UNUSED)
{
   Eina_Content_Convertion_Node *n = data;

   return n->to;
 }

EAPI Eina_Iterator*
eina_content_converter_possible_converstions(const char *from)
{
   Eina_List *possibilities = _convertion_callback_fetch_possible(from);
...
{
   Eina_Content_Convertion_Callback callback = _convertion_callback_fetch(content->type, new_type);

   if (!callback)
     {
        ERR("No suitable convertion found");
        return NULL;
     }
   ...
   if (convert == EINA_VALUE_TYPE_STRINGSHARE ||
       convert == EINA_VALUE_TYPE_STRING)
     {
        ...
        if (eina_streq(type, "text/plain;charset=utf-8"))
          ...
          {
             Eina_Iterator *iter = eina_content_possible_converstions(*ra);
             const char *type;
...
eina_abstract_content_init(void)
{
   ...
   convertion_callbacks = eina_hash_stringshared_new(_free_node);
...
{
   eina_hash_free(convertion_callbacks);

   return EINA_TRUE;
}

Eina_Value*
eina_value_content_new(Eina_Content *value)
{
   Eina_Value *v = eina_value_new(EINA_VALUE_TYPE_CONTENT);

   eina_value_pset(v, &value);
   eina_content_free(value);
   return v;
}


Eina_Value
eina_value_content_init(Eina_Content *value)
{
   Eina_Value v;
